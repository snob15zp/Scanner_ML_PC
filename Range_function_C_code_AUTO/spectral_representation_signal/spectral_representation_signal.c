/*
 * File: spectral_representation_signal.c
 *
...
#include "spectral_representation_signal_emxutil.h"
 ...
 * Return Type  : void
 */
void spectral_representation_signal(const double Signal[2250001], double FftL,
  emxArray_real_T *FftS)
{
  ...
  eml_fft(Signal, (int)FftL, b_y1);
  k = FftS->size[0] * FftS->size[1];
  FftS->size[0] = 1;
  FftS->size[1] = (int)FftL;
  emxEnsureCapacity((emxArray__common *)FftS, k, sizeof(double));
  for (k = 0; k + 1 <= (int)FftL; k++) {
    x_re = b_y1->data[k].re;
    x_im = b_y1->data[k].im;
    FftS->data[k] = rt_hypotd_snf(x_re, x_im);
  }
  ...
  b_FftS *= k;
  for (k = 0; k < b_FftS; k++) {
    FftS->data[k] = 2.0 * FftS->data[k] / FftL;
  }
...
/*
 * File trailer for spectral_representation_signal.c
 *
 * [EOF]
 */
